package com.xiaosuokeji.aocai.home.util;
...
import com.xiaosuokeji.aocai.home.model.CachedMap;
...
public class WebUtils {
    private static CachedMap cachedData = new CachedMap();
     ...
     * @return 请求结果Json对象
     */
    public static JSONObject doRawRequest(API api, String json, Boolean cache) {
        JSONObject result = null;
        if (cache) {
            try {
                result = (JSONObject) cachedData.get(api.name() + Md5Util.encode(json));
        ...
        try {
            ...
            if (resultStr != null) {
                ...
                if (result.getBoolean("status") && cache)
                    cachedData.put(api.name() + Md5Util.encode(json), result);
                return result;
            } else
                return new JSONObject("{\"status\": false,\"code\": -1,\"msg\": \"请求失败\"}");
        } catch (Exception e) {
    ...
    public static JSONObject doRawRequest(API api, String json) {
        return doRawRequest(api, json, false);
    }
    ...
    public static JSONObject doRawRequest(API api, Object body) {
        return doRawRequest(api, GsonUtils.toJson(body), false);
    }

    public static JSONObject doRawRequest(API api, Object body, Boolean cache) {
        return doRawRequest(api, body, cache);
    }
